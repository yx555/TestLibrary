...
NSString const *UIButton_isDeveloperKey = @"UIButton_isDeveloperKey";
...
-(void) setBadgeValue:(NSString *)badgeValue
{
    ...
    if (!badgeValue || [badgeValue isEqualToString:@""] || self.shouldHideBadgeAtZero) {
        ...
        if ([@"0" isEqualToString:badgeValue]) {
            ...
            if (isValid(self.isDeveloper)&&[@"0" isEqualToString:self.isDeveloper]) {
                
                self.badge.frame                = CGRectMake(self.frame.size.width-6,6,8,8);
            }else{
               self.badge.frame                = CGRectMake(self.frame.size.width-8,0,8,8);
            }
...
- (NSString *)isDeveloper{
    
    return objc_getAssociatedObject(self, &UIButton_isDeveloperKey);
}
- (void)setIsDeveloper:(NSString *)isDeveloper{
    objc_setAssociatedObject(self, &UIButton_isDeveloperKey, isDeveloper, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
@end
